...
async def deepseek_v3_complete(
    ...
    # 确保从配置或环境变量中获取 API 密钥
    api_key = kwargs.get("api_key", os.getenv("OPENAI_API_KEY"))
    if not api_key:
        raise ValueError("Missing DeepSeek API key. Please set DEEPSEEK_API_KEY.")

    # 初始化 DeepSeek 客户端
#     print("DEEPSEEK OPENAI_API_KEY",api_key)
...
# ) -> str:
#     """
#     调用 DeepSeek v3 模型进行文本生成。

#     :param prompt: 用户输入的提示词
#     :param system_prompt: 系统提示词（可选）
#     :param history_messages: 上下文消息历史（可选）
#     :param keyword_extraction: 是否启用关键词提取模式（可选）
#     :param kwargs: 其他参数
#     :return: 模型生成的文本
#     """
#     # 从环境变量或 kwargs 获取 DeepSeek API 密钥
#     default_deepseek_api_key = "sk-eb26a5a560d74d308ac772be37b2bc15"
#     api_key = kwargs.get("api_key", os.getenv("OPENAI_API_KEY", default_deepseek_api_key))
...
@wrap_embedding_func_with_attrs(embedding_dim=1024, max_token_size=8192)
async def ollama_embed(texts: list[str], model: str, **kwargs) -> np.ndarray:
    """
    调用 Ollama 模型的嵌入函数。
