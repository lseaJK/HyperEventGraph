...
class HybridRetrieverAgent:
    ...
    def __init__(self, storage_agent: StorageAgent):
        ...
        它会复用StorageAgent的数据库连接和模型来初始化底层的HybridRetriever。
        """
        print("Initializing Hybrid Retriever Agent...")
        self.retriever = HybridRetriever(
            neo4j_driver=storage_agent.neo4j_driver,
            chroma_client=storage_agent.chroma_client,
            embedding_model=storage_agent.embedding_model
        )
    ...
    def retrieve_context(self, text_to_analyze: str, top_k: int = 3) -> str:
        ...
        Args:
            text_to_analyze (str): 需���分析并为其查找上下文的文本。
