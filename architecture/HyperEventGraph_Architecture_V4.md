...
3.  **学习阶段 (事件中心型定义)**: 专家运行`run_learning_workflow.py`。工作流的核心是**事件级聚类**：系统首先从所有未知文档中抽取出原子事件摘要，然后对这些摘要进行聚类。专家通过交互式命令，对这些**事件簇**进行审核、合并，最终定义出新的Schema。
4.  **闭环阶段 (学习、抽取与应用)**: 新Schema保存后，系统会找到贡献了该事件簇的所有**原始文档**，并**立即、自动地**将新学习到的Schema应用于这些文档上，进行一次目标明确的事件抽取。
