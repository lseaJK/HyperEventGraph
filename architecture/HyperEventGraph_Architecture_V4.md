...
2.  **触发阶段 (批量学习)**: 当`pending_learning`的事件达到**学习阈值**（在`config.yaml`中配置）或由项目经理**手动决定**时，触发学习。
3.  **学习阶段 (交互式定义)**: 专家运行`run_learning_workflow.py`，与`SchemaLearnerAgent`协同，为新的事件簇定义Schema。
4.  **闭环阶段 (知识应用与再处理)**: 新Schema保存后，`run_learning_workflow.py`脚本自动将所有刚用于学习的事件状态**重置为`pending_triage`**。这使得它们能被系统用新的“知识视角”重新识别和处理，完成螺旋式上升的知识增长。
